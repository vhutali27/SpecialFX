...
//////////////////////////////////////////////////
// Global Variables	For ONLINE SESSION		//
//////////////////////////////////////////////////
...
Physijs.scripts.worker = '/ExternalLibraries/physijs_worker.js';
Physijs.scripts.ammo = '/ExternalLibraries//ammo.js';
...
class OnlinePlayer{
	constructor(){
		...
		loaderMTL.load("static/SpecialFX_online/models/Gun/gun.mtl", function ( materials ) {
			materials.preload();
			var loaderOBJ = new THREE.OBJLoader();
			loaderOBJ.setMaterials(materials);
			loaderOBJ.load(
				"static/SpecialFX_online/models/Gun/gun.obj",
				function (object) {
		...
		// Load a glTF resource
			loaderGLTF.load(
				// resource URL
				'static/SpecialFX_online/models/character.glb',
...
class Player{
	...
	getPosition(){
		return {
			'position' : this.Group.position // Return the players position
		}
	}
}
...
map: loader.load("static/SpecialFX_online/images/woodenFloor.jpg"),
color: 0x72f2f2,
specular: 0xbbbbbb,
shininess: 2
});
...
Surface1.addObjObject("static/SpecialFX_online/models/bucket/bucket.obj","static/SpecialFX_online/models/bucket/bucket.mtl", true, 75, 75, 2, {x:0.15,y:0.15,z:0.15}, Surface1);

//earth.addObject(getSquare(earthMaterial, 2),23,1,2);

// Mars
var grassMaterial = new THREE.MeshPhongMaterial({
map: loader.load("static/SpecialFX_online/images/grass.jpg"),
color: 0x464742,
specular: 0xbbbbbb,
shininess: 2
});
...
Surface2.addObjObject("static/SpecialFX_online/models/steel_fence/fance.obj","static/SpecialFX/models/steel_fence/fance.mtl",false,0,0,0, {x:5,y:5,z:5},Surface2);
Surface2.addObjObject("static/SpecialFX_online/models/birch/birch.obj","static/SpecialFX/models/birch/birch.mtl", false, 75, 75, 0, {x:0.15,y:0.15,z:0.15},Surface2);

//Stars
var starGeometry = new THREE.SphereGeometry(1000, 50, 500);
var starMaterial = new THREE.MeshPhongMaterial({
map: loader.load("static/SpecialFX_online/images/galaxy_starfield.png"),
...
function initOnlineSession(){
	...
	$(document).ready(function(){
		$.ajax({
			url:'online/ajax/init/',
			data:{
				'name':'tyler', 
				'position': String(playerPosition['position']['x']) + "," + String(playerPosition['position']['y']) + "," + String(playerPosition['position']['z']),
			},
			dataType:'json',
			success: function(player_number){
...
//////////////////////////////////////////////////
// var online_player = new OnlinePlayer();
// AnimateObject.push(online_player)
// online_player.setTargetPlanet(Surface2)
var OnlinePlayers = {}
function updateOnlinePositions(positions_){
	console.log(positions_)
	for (var player_number_ in positions_){
		if (String(player_number_) === String(playerNumber['player_number'])) // Our position has already 	
			continue
		if (! (player_number_ in OnlinePlayers)){
			OnlinePlayers[player_number_] = new OnlinePlayer()
			AnimateObject.push(OnlinePlayers[player_number_])
			OnlinePlayers[player_number_].setTargetPlanet(Surface1)
			OnlinePlayers[player_number_].rotate();
			
		}
		positions = String(positions_[player_number_]).split(",");
		console.log("Server Positions", positions, "\n\n");
		OnlinePlayers[player_number_].updatePosition(positions[0], positions[1], positions[2]);
			
	}
}

function getOnlinePositions(playerPosition){
	var playerPosition = player.getPosition();
	$(document).ready(function(){
		$.ajax({
			url:'online/ajax/data/',
			data:{
				'player_number': String(playerNumber),
				'position': String(playerPosition['position']['x']) + "," + String(playerPosition['position']['y']) + "," + String(playerPosition['position']['z']),
			},
			dataType: 'json',
			success: function(positions_){
					updateOnlinePositions(positions_);
			}
		});
	});
}
